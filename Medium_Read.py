...
class hydro_reader:
	def __init__(self, hydro_file_path):
		...
		for i in range(self.Nstep):
			self.T.append( self.file['Event'][self.step_keys[i]]['Temp'][()] )
			self.Vx.append( self.file['Event'][self.step_keys[i]]['Vx'][()] )
			self.Vy.append( self.file['Event'][self.step_keys[i]]['Vy'][()] )
		self.file.close()	
