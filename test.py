...
plt.figure()
plt.plot(list_tau, im_e, linewidth = 2.0, color='black', label='numerical')
plt.plot(list_tau, analytic_im, linewidth = 2.0, color='red', linestyle = '--', label='analytic')
plt.xlabel(r'$\tau$(GeV$^{-1}$)', size = 20)
plt.ylabel(r'$\Im\delta\tilde{\epsilon}$(GeV)', size = 20)
plt.legend(loc='best', fontsize = 16)
plt.xticks(fontsize = 16)
plt.yticks(fontsize = 16)
plt.savefig('test_Im_e_kx='+str(kx)+'_ky='+str(ky)+'_x0='+str(x0)+'_y0='+str(y0)+'.pdf')
plt.show()